...
После запуска напишите боту в Telegram — ответ идёт через mock LLM и простой retrieval. Для ответов по базе знаний сначала выполните `make reingest`. Orchestrator использует порог уверенности RAG (`ORCHESTRATOR_RAG_MIN_CONFIDENCE`, по умолчанию 0.30): при низкой уверенности поиска возвращается режим диагностики (уточняющие вопросы и рекомендации что собрать) без вызова LLM.
